...
const UsersWalletReports = () => {
  ...
  const getProductData = async () => {
    ...
    let credit = [];
    let debits = [];
    let all_wallet_data = await fetch(
      "https://krushimitr.in/api/users/get-rupee-wallet"
    );
    const all_users_wallets = await all_wallet_data.json();

    if (all_users_wallets.status == 201) {
      all_users_wallets.result.map((item) => {
        if (item.type === "Credit") {
          credit.push(item);
      ...
      console.log(all_users_wallets.result);
    }
  };
...
export default UsersWalletReports;
