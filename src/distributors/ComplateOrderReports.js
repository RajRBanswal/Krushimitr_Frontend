  ...
  const getProductData = async () => {
    ...
    if (all_orders.status === 201) {
      all_orders.result.map((item) => {
        if (distributor === item.vendorId && item.orderStatus === "Done") {
          data.push(item);
        }
      });
      setFilterData(data);
    ...
    <div className="py-2">
      <div className="row">
        <div className="col-lg-4">
          <h4 className="m-0">Completed Report</h4>
        </div>
...
export default ComplateOrderReports;
